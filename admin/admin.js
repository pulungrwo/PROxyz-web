(() => {
  ...
  function authHeaders(headers = {}) {
    // Sesi Admin memakai cookie HttpOnly dari api.pulungriswanto.my.id.
    // Token tidak disimpan di localStorage agar tidak dapat dicuri oleh script halaman.
    return { ...headers };
  }
  ...
  const COMPETITION_FORMAT_HELP = {
    round_robin:"Semua peserta saling bertemu. Juara ditentukan dari klasemen akhir.",
    groups_knockout:"Peserta dibagi ke grup, lalu peserta terbaik lanjut ke bagan gugur.",
    single_elimination:"Kalah langsung tersingkir hingga tersisa juara."
  };
  ...
  function renderCompetition() {
    ...
    $("competition-match-count").textContent=wholeNumber.format((d.matches||[]).length); $("competition-finished-count").textContent=wholeNumber.format((d.matches||[]).filter(row=>row.status==="finished").length);
    ...
    fillCompetitionOptionList($("competition-setting-format"),COMPETITION_FORMATS); $("competition-setting-name").value=d.nama||""; $("competition-setting-sport-name").value=d.cabangNama||""; $("competition-setting-format").value=COMPETITION_FORMATS.some(row=>row[0]===d.format)?d.format:"single_elimination"; $("competition-setting-groups").value=String(d.groupCount||2); $("competition-setting-advance").value=String(d.advancePerGroup||2); $("competition-setting-legs").value=String(Math.min(2,Math.max(1,Number(d.legs||1)))); $("competition-setting-scoring").value=d.scoringMode||"score"; $("competition-setting-bestof").value=String(d.bestOf||1); $("competition-setting-draw").checked=d.allowDraw!==false; $("competition-setting-third").checked=d.thirdPlace!==false; syncCompetitionSettingOptions();
    const standingsTab=$("competition-tab-klasemen"), bracketTab=$("competition-tab-bagan"); if(standingsTab)standingsTab.hidden=d.format==="single_elimination"; if(bracketTab)bracketTab.hidden=d.format==="round_robin"; if((activeCompetitionTab==="klasemen"&&standingsTab?.hidden)||(activeCompetitionTab==="bagan"&&bracketTab?.hidden))activeCompetitionTab="pertandingan";
  ...
  function renderCompetitionMatches() {
    ...
    const list=$("competition-match-list");list.replaceChildren();const rows=d.matches||[];if(!rows.length){list.appendChild(emptyBox("Belum ada jadwal. Tambahkan peserta lalu tekan Buat jadwal."));return;}
  ...
  function standingsTable(rows,title="Klasemen") {const wrap=document.createElement("section");wrap.className="competition-table-card";const h=document.createElement("h3");h.textContent=title;wrap.append(h);const table=document.createElement("div");table.className="competition-table";const head=document.createElement("div");head.className="competition-table-row head";["#","Peserta","M","W","D","L","+/−","Poin"].forEach(x=>{const span=document.createElement("span");span.textContent=x;head.append(span);});table.append(head);rows.forEach((r,i)=>{const row=document.createElement("div");row.className="competition-table-row";[i+1,r.name,r.main,r.win,r.draw,r.loss,r.diff,r.points].forEach((x,j)=>{const span=document.createElement("span");span.textContent=x;if(j===1)span.className="name";row.append(span);});table.append(row);});wrap.append(table);return wrap;}
  ...
  function syncCompetitionCreateOptions(){const format=$("competition-create-format").value;$("competition-create-group-options").hidden=format!=="groups_knockout";$("competition-create-custom-sport-wrap").hidden=$("competition-create-sport").value!=="lainnya";$("competition-create-system-help").textContent=COMPETITION_FORMAT_HELP[format]||"";}
  function syncCompetitionSettingOptions(){const format=$("competition-setting-format").value;$("competition-setting-group-options").hidden=format!=="groups_knockout";$("competition-setting-league-options").hidden=format!=="round_robin";$("competition-setting-third-wrap").hidden=format==="round_robin";$("competition-setting-draw-wrap").hidden=$("competition-setting-scoring").value!=="score";$("competition-setting-system-help").textContent=COMPETITION_FORMAT_HELP[format]||"";}
  ...
  function renderRisma() {
    ...
    for (const id of ["risma-publish-announce-preview","risma-publish-announce-pdf","risma-publish-invite-pdf"]) setDisabled(id, !period);
  ...
  function appendRismaRankCutoff(list, label) {
    ...
    divider.className="risma-rank-cutoff";
  ...
  function renderRismaRankings() {
    ...
    if (!teams.length) teamList.appendChild(emptyBox("Tim belum disusun. Tim awal dibuat setelah input Minggu 1."));
      ...
      const teamLimit=Number(rismaDetail?.settings?.teamWinnerCount||3);
      if(index+1===teamLimit && teams.length>teamLimit) appendRismaRankCutoff(teamList,`Batas penerima penghargaan · Top ${teamLimit}`);
    });
  }
  ...
  function renderRismaScores() {
    ...
    rows.forEach((row,index)=>{
      ...
      const name=document.createElement("strong"); name.className=`risma-row-name${row.disqualified?" risma-disqualified-name":""}`; name.textContent=row.disqualified?`— ${row.name}`:`${index+1}. ${row.name}`;
      ...
      const meta=document.createElement("div"); meta.className="item-meta risma-row-meta"; meta.textContent=`${participantWeekSummary(row)}${row.gender?` · ${row.gender}`:""}${row.excusedUsed?` · berhalangan ${row.excusedUsed}/7 hari`:""}`;
  ...
  function syncRismaWeekBuilder() {
    ...
    if (week === 1) {
      ...
      const selectedGender=String(selectedParticipant?.gender||editing?.gender||$("risma-week-gender").value||"");
      excusedWrap.hidden=selectedGender!=="perempuan";
      label.firstChild.textContent = "Nama peserta\n            ";
      $("risma-week-entry-hint").textContent = excusedWrap.hidden
        ? "Masukkan nama, jenis peserta, dan jumlah hadir."
        : "Peserta perempuan. Pilih jumlah hari berhalangan bila diperlukan.";
    } else {
      toggle.hidden = false;
      if (rismaWeekNewParticipantMode) {
        nameInput.hidden = false; select.hidden = true; genderWrap.hidden=false;
        const editing=rismaWeekEditIndex>=0?rismaWeekDraft[rismaWeekEditIndex]:null;
        const selectedGender=String(editing?.gender||$("risma-week-gender").value||"");
        excusedWrap.hidden=selectedGender!=="perempuan";
        toggle.textContent = "Pilih peserta lama";
        label.firstChild.textContent = "Nama peserta baru\n            ";
        $("risma-week-entry-hint").textContent = excusedWrap.hidden
          ? "Peserta baru akan dibuatkan tim baru agar tim lama tidak berubah."
          : "Peserta perempuan baru. Berhalangan dapat langsung diisi; tim lama tetap tidak berubah.";
      } else {
        ...
        $("risma-week-entry-hint").textContent = excusedWrap.hidden
          ...
          : `Peserta perempuan. Berhalangan tersisa ${Math.max(0,7-Number(selectedParticipant?.excusedUsed||0))} hari.`;
      }
    }
    if(!excusedWrap.hidden){
      const oldExcused=rismaWeekEditIndex>=0?Number(rismaWeekDraft[rismaWeekEditIndex]?.excused||0):0;
      const usedBefore=selectedParticipant?Number(selectedParticipant.excusedUsed||0):0;
      const remaining=Math.max(0,7-(usedBefore-oldExcused));
  ...
  function renderRismaWeekDraft() {
    ...
    rismaWeekDraft.forEach((row,index)=>{
      ...
      const meta=document.createElement("span"); meta.textContent=`${row.attendance}x hadir${row.excused?` + ${row.excused} hari berhalangan`:""} → ${number.format(rismaPointForAttendance(effective))} poin${row.isNew ? " · peserta baru" : ""}`;
      info.append(title,meta);
  ...
  function addRismaWeekDraft() {
    ...
    if(week===1 || rismaWeekNewParticipantMode){
      ...
      if(String(gender)==="perempuan") {
        excused=Number($("risma-week-excused").value||0);
        const usedBefore=known?Number(known.excusedUsed||0):0;
        const oldExcused=rismaWeekEditIndex>=0?Number(rismaWeekDraft[rismaWeekEditIndex]?.excused||0):0;
        const remaining=Math.max(0,7-usedBefore+oldExcused);
        if(excused>remaining){setStatus($("risma-week-status"),`Berhalangan ${name} tersisa ${remaining} hari.`,"error");return;}
      }
    } else {
  ...
  function renderRismaCoupons(){
    ...
    for(const row of group.rows){
      ...
      const done=rismaIconButton(row.status==="done"?"Batal":"Selesai",row.status==="done"?"fa-rotate-left":"fa-check","ghost compact risma-mini-action risma-coupon-status-action",()=>toggleRismaCoupon(row));
      ...
      const automatic=Boolean(row.autoSource);
      if(automatic){edit.disabled=true;del.disabled=true;edit.title="Kupon otomatis mengikuti Rank Minggu 4.";del.title=edit.title;}
      line.append(name,count,done,edit,del);
      const meta=document.createElement("div"); meta.className="item-meta risma-row-meta"; meta.textContent=`${row.status==="done"?"Sudah dibagikan":"Belum dibagikan"}${automatic?" · Otomatis · Rank Minggu 4":""}`;
  ...
  async function shareRismaText(){
    ...
    window.location.href=`whatsapp://send?text=${encodeURIComponent(draft.text)}`;
  }
...
Nomor surat yang terhapus dapat dipakai kembali jika menjadi nomor terakhir.`))return;
  ...
  async function saveRismaSignatures(){
    ...
    const advisor=$("risma-signature-advisor").value.trim();
    setStatus($("risma-signature-status"),"Menyimpan penandatangan…");
    try{
      const data=await api("/api/risma/signatures",{method:"PUT",body:JSON.stringify({chair,advisor})});
  ...
  function renderRismaPeriodHistory(){
    ...
    for(const row of rows){
      ...
      side.append(chip,view);
      if(rismaDetail?.role==="owner"){
        const del=button("Hapus","danger-soft compact",()=>deleteRismaPeriod(row));del.innerHTML='<i class="fa-solid fa-trash-can"></i> Hapus';side.appendChild(del);
      }
      card.append(info,side);list.appendChild(card);
    }
  }
  ...
  async function deleteRismaPeriod(row){
    if(rismaDetail?.role!=="owner")return;
    const phrase=`HAPUS ${row.hijriYear}`;
    const typed=prompt(`Hapus permanen periode Ramadan ${row.hijriYear} H beserta poin, tim, kupon, dan publikasinya?\n\nBackup pemulihan akan disimpan di Aktivitas Owner.\nKetik: ${phrase}`);
    if(String(typed||"").trim().toUpperCase()!==phrase)return;
    setStatus($("risma-period-history-status"),`Menghapus Ramadan ${row.hijriYear} H…`);
    try{
      const data=await api(`/api/risma/period/${encodeURIComponent(row.id)}`,{method:"DELETE",body:"{}"});
      await loadRisma();
      setStatus($("risma-period-history-status"),data.message||"Periode dihapus. Backup tersimpan di Aktivitas.","success");
    }catch(error){setStatus($("risma-period-history-status"),error.message,"error");}
  }
  ...
  function renderRismaHistoricalSummary(period){
    ...
    for(const [label,value] of [["Peserta",period.participants],["Minggu",`${period.weeks}/4`],["Tim",period.teams],["Kupon",period.coupons]]){
      const card=document.createElement("article");card.className="metric-card";
      const labelEl=document.createElement("span");labelEl.textContent=String(label);
      const valueEl=document.createElement("b");valueEl.textContent=String(value);
      card.append(labelEl,valueEl);metrics.appendChild(card);
    }
    ...
    else(period.scores||[]).forEach((row,index)=>{
      const card=document.createElement("article");card.className=`risma-history-rank-row${row.disqualified?" disqualified":""}`;
      const name=document.createElement("b");if(row.disqualified)name.className="risma-disqualified-name";name.textContent=`${row.disqualified?"—":`${index+1}.`} ${row.name}`;
      const meta=document.createElement("span");meta.textContent=`${participantWeekSummary(row)}${row.disqualified?" · DISKUALIFIKASI":""}`;
      const points=document.createElement("strong");points.textContent=`${number.format(row.totalPoints||0)} poin`;
      card.append(name,meta,points);participants.appendChild(card);
    });
    ...
    else(period.teamRows||[]).forEach((row,index)=>{
      const card=document.createElement("article");card.className="risma-history-team-row";
      const name=document.createElement("b");name.textContent=row.disqualified?`— ${row.name}`:`${index+1}. ${row.name}`;
      const meta=document.createElement("span");meta.textContent=`Ø ${number.format(row.averagePoints||0)} poin · Total ${number.format(row.totalPoints||0)} poin`;
      const members=document.createElement("small");members.textContent=(row.members||[]).map(x=>x.name).join(" · ");
      card.append(name,meta,members);teams.appendChild(card);
    });
    const coupons=$("risma-period-summary-coupons");coupons.replaceChildren();
    for(const row of period.couponTypes||[]){
      const card=document.createElement("article");card.className="metric-card";
      const label=document.createElement("span");label.textContent=String(row.label||"");
      const total=document.createElement("b");total.textContent=wholeNumber.format(row.stats?.coupons||0);
      const recipients=document.createElement("small");recipients.textContent=`${wholeNumber.format(row.stats?.recipients||0)} penerima`;
      card.append(label,total,recipients);coupons.appendChild(card);
    }
...
${row.label}`))return;
  ...
  function clearRismaInvitationDraftAfterIssueV159(){
    try{ localStorage.removeItem(RISMA_INVITATION_DRAFT_KEY_V156); }
    catch(error){ console.warn("[RISMA] Draft undangan tidak dapat dihapus:", error?.message || error); }

    const fields=[
      "risma-publish-invite-event",
      "risma-publish-invite-date",
      "risma-publish-invite-time",
      "risma-publish-invite-place",
      "risma-publish-invite-note"
    ];
    fields.forEach(id=>{
      const el=$(id);
      if(!el)return;
    ...
    rismaInvitationLetterNumber="";
    rismaInvitationDraftKey="";
  }
  ...
  async function printRismaInvitationV156(){
    ...
    const required = [
      ["risma-publish-invite-event","Kegiatan"],
      ["risma-publish-invite-date","Tanggal"],
      ["risma-publish-invite-time","Waktu"],
      ["risma-publish-invite-place","Tempat"]
    ];
    const missing = required.filter(([id])=>!String($(id)?.value || "").trim()).map(([,label])=>label);
    if(missing.length){
      setStatus(status,`Lengkapi: ${missing.join(", ")}.`,"error");
      return;
    }

    if(!confirm("Terbitkan undangan ini sekarang?\n\nPDF akan langsung dikirim ke WhatsApp admin. Tidak ada file yang di-download ke perangkat. Setelah berhasil, undangan masuk Arsip Publikasi dan menu Bagikan muncul.")) return;

    button.disabled=true;
    setStatus(status,"Menerbitkan undangan dan mengirim PDF ke WhatsApp admin…");
    const issueData=rismaPublicationPayload("invitation");

    try{
      const result=await api("/api/risma/publication/pdf-self",{
        method:"POST",
        body:JSON.stringify({kind:"invitation",data:issueData})
      });

      if(result.archive) upsertRismaArchive(result.archive);
      clearRismaInvitationDraftAfterIssueV159();

      const published=$("risma-publish-invite-published");
      if(published){
        published.hidden=false;
        published.textContent="✓ UNDANGAN TERBIT — PDF sudah dikirim ke WhatsApp admin dan arsip telah diperbarui.";
        published.className="status success";
      }

      setStatus(status,result.message || "Undangan berhasil diterbitkan.","success");

      openRismaShareDialog({
        kind:"invitation",
        data:issueData,
        title:result.title || ("Undangan · "+(issueData.event || "RISMA")),
        text:result.text || "",
        letterNumber:result.letterNumber || issueData.letterNumber || "",
        event:result.event || issueData.event || "",
  ...
  async function runRismaInviteDistribution(action){
    const status=$("risma-publish-invite-status");
    try{
      const payload={kind:"invitation",data:rismaPublicationPayload("invitation")};
      if(action==="text-person"){
        const r=await api("/api/risma/publication/preview",{method:"POST",body:JSON.stringify(payload)});
        if(navigator.share){await navigator.share({title:r.title || "Undangan RISMA",text:r.text || ""});}
        else window.open("https://wa.me/?text="+encodeURIComponent(r.text || "" ),"_blank","noopener,noreferrer");
        setStatus(status,"Teks undangan siap dibagikan.","success"); return;
      }
      if(action==="pdf-person"){
        const r=await apiRaw("/api/risma/publication/file",{method:"POST",body:JSON.stringify(payload)});
        if(!r?.ok) throw new Error("PDF undangan gagal dibuat.");
        const blob=await r.blob();
        const file=new File([blob],"undangan-risma.pdf",{type:"application/pdf"});
        if(navigator.share && (!navigator.canShare || navigator.canShare({files:[file]}))){await navigator.share({title:"Undangan RISMA",files:[file]});setStatus(status,"PDF undangan siap dibagikan.","success");return;}
        throw new Error("Perangkat/browser tidak mendukung Share PDF.");
      }
      const endpoint=action==="pdf-group"?"/api/risma/publication/pdf-group":"/api/risma/publication/share-group";
      const result=await api(endpoint,{method:"POST",body:JSON.stringify(payload)});
      setStatus(status,result.message || "Distribusi berhasil.","success");
    }catch(error){if(error?.name!=="AbortError")setStatus(status,error?.message || "Distribusi gagal.","error");}
  }
  ...
  const invitePdfGroup=$("risma-publish-invite-pdf-group");
  const inviteTextGroup=$("risma-publish-invite-text-group");
  const invitePdfPerson=$("risma-publish-invite-pdf-person");
  const inviteTextPerson=$("risma-publish-invite-text-person");
  if(invitePdfGroup) invitePdfGroup.onclick=()=>runRismaInviteDistribution("pdf-group");
  if(inviteTextGroup) inviteTextGroup.onclick=()=>runRismaInviteDistribution("text-group");
  if(invitePdfPerson) invitePdfPerson.onclick=()=>runRismaInviteDistribution("pdf-person");
  if(inviteTextPerson) inviteTextPerson.onclick=()=>runRismaInviteDistribution("text-person");
